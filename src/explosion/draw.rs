...
impl Drawable {
    ...
    pub fn update(&mut self, world_to_clip: &Matrix4<f32>, pos: &Position, radius: f32) {
        self.locals.transform = (world_to_clip * pos.model_to_world()).into();
        self.locals.radius = radius;
    }
}
...
static VERTICES: [Vertex; 4] = [Vertex { pos: [-0.5, 0.5] },
                                Vertex { pos: [0.5, 0.5] },
                                Vertex { pos: [-0.5, -0.5] },
                                Vertex { pos: [0.5, -0.5] }];
const SHADER_VERT: &'static [u8] = include_bytes!("explosion.v.glsl");
const SHADER_FRAG: &'static [u8] = include_bytes!("explosion.f.glsl");
...
impl<C> specs::System<C> for PreDrawSystem {
    fn run(&mut self, arg: specs::RunArg, _: C) {
        use specs::Join;
        let (positions, explosives, dim, mut projectiles) =
            arg.fetch(|w| {
                          (w.read::<Position>(),
                           w.read::<Explosion>(),
        ...
        for (p, e, d) in (&positions, &explosives, &mut projectiles).join() {
            d.update(&world_to_clip, p, e.radius());
        }
    }
}
